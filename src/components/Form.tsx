...
const Form = ({ type, post, submitting, setPost, handleSubmit }: IPromptForm) => {
  return (
    <section className='w-full max-w-full flex-start flex-col'>
      ...
      >
        <label>
          ...
          <textarea
            value={post.prompt}
            onChange={(data) => setPost({ ...post, prompt: data.target.value })}
        ...
        <label>
          ...
          <input
            value={post.tags}
            onChange={(data) => setPost({ ...post, tags: data.target.value })}
            placeholder='#tag'
            required
